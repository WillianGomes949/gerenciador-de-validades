...
export default function HomePage() {
  ...
  // Calcula status de validade com cor, texto e tooltip
  ...
  // Ordena produtos por validade e quantidade
  const sortProducts = (list) => {
    return [...list].sort((a, b) => {
      const dateA = new Date(a.validade); const dateB = new Date(b.validade);
  ...
  useEffect(() => {
    ...
    } else setFormState(INITIAL_FORM_STATE);
  }, [editingProduct]);

  const isFormValid = () => {
    if (!formState.validade || isNaN(new Date(formState.validade))) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingProduct) {
        res = await updateProduct(formState);
        setProducts(prev => sortProducts(prev.map(p => p.id === formState.id ? formState : p)));
      } else {
        res = await addProduct(formState);
        setProducts(prev => sortProducts([...prev, formState]));
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert(res?.message || 'Produto excluído!');
    } catch (error) {
          ...
          <form onSubmit={handleSubmit}>
            <fieldset disabled={isSubmitting} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 flex gap-4 mt-2">
                ...
                {editingProduct && (
                  <button type="button" onClick={() => setEditingProduct(null)} className="p-3 bg-slate-500 text-white font-bold rounded-md hover:bg-slate-600">Cancelar</button>
                )}
              </div>
            </fieldset>
          </form>
                ...
                return (
                  <div key={product.id} className="grid grid-cols-2 md:grid-cols-6 gap-4 items-center p-4 bg-slate-50 border rounded-md hover:shadow-md transition-shadow">
                    ...
                    <div className="text-center md:col-span-1">
                      ...
                      <p className={status.cor} data-tooltip-id={`tooltip-${product.id}`} data-tooltip-content={status.tooltip}>{status.texto}</p>
